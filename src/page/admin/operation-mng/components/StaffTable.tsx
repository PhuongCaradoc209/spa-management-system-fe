...
const buildName = (raw: Record<string, unknown>) => {
  ...
  return [first, last].filter(Boolean).join(" ") || String(raw.fullName ?? raw.name ?? "-");
};
...
export const StaffTable: React.FC = () => {
  ...
  const rows = extractUsers(data).map((raw) => ({
    id: String(raw.id ?? raw.userId ?? raw._id ?? ""),
    name: buildName(raw),
    role: String(raw.role ?? "").toUpperCase(),
    isActive: raw.isActive as boolean | undefined,
    isAvailable: raw.isAvailable as boolean | undefined,
  })).filter((r) => r.id && (r.role === "STAFF" || r.role === "THERAPIST" || r.role === ""));
  ...
  return (
    <section className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button className="px-4 py-2 bg-primary text-white rounded-full text-xs font-bold">Add Staff</button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-surface-container-low">
              ...
              <th className="px-6 py-5 text-[10px] font-label tracking-[0.15em] text-on-surface-variant/40 uppercase">Role</th>
          ...
          <tbody className="divide-y divide-outline-variant/10">
            ...
            {!isLoading && !isError && rows.length === 0 && <tr><td className="px-6 py-10 text-center text-sm text-on-surface-variant/60" colSpan={3}>No staff found.</td></tr>}
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-surface-container-low/40 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-on-surface-variant">{row.role || "-"}</td>
                <td className="px-6 py-4">
                  {row.isActive === false ? <StatusPill label="Inactive" variant="muted" />
                    : row.isAvailable === false ? <StatusPill label="Unavailable" variant="warn" />
